...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const usdcBal = typeof usdcBalanceRaw === "bigint" ? Number(usdcBalanceRaw) / 1_000_000 : 0;
    setUsdcBalance(usdcBal);

    // Calculate BTC token balances (all have 18 decimals except WBTC which has 8)
    const cbbtcBal = typeof cbbtcBalanceRaw === "bigint" ? Number(cbbtcBalanceRaw) / 100_000_000 : 0;
    const lbtcBal = typeof lbtcBalanceRaw === "bigint" ? Number(lbtcBalanceRaw) / 1_000_000_000_000_000_000 : 0;
    const wbtcBal = typeof wbtcBalanceRaw === "bigint" ? Number(wbtcBalanceRaw) / 100_000_000 : 0; // WBTC has 8 decimals
        ...
        <main className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm">
              <h3 className="text-lg font-semibold text-gray-700 mb-6">Total Amount Invested</h3>
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-4xl font-bold text-gray-900 mb-2">
                    ${(tokenBalances.usdc + (tokenBalances.cbbtc + tokenBalances.lbtc + tokenBalances.wbtc) * 95000).toLocaleString()}
                  </div>
                  <div className="text-lg text-gray-600">
                    ≈ {(tokenBalances.cbbtc + tokenBalances.lbtc + tokenBalances.wbtc).toFixed(8)} BTC
                  </div>
                </div>
                  ...
                  <div className="w-full h-full rounded-full border-8 border-blue-500 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-500">
                        {((tokenBalances.cbbtc + tokenBalances.lbtc + tokenBalances.wbtc) * 95000 / 
                          (tokenBalances.usdc + (tokenBalances.cbbtc + tokenBalances.lbtc + tokenBalances.wbtc) * 95000) * 100).toFixed(1)}%
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 flex items-center space-x-4 text-sm">
                <div className="flex items-center space-x-2">
                  ...
                  <span>BTC {((tokenBalances.cbbtc + tokenBalances.lbtc + tokenBalances.wbtc) * 95000 / 
                    (tokenBalances.usdc + (tokenBalances.cbbtc + tokenBalances.lbtc + tokenBalances.wbtc) * 95000) * 100).toFixed(1)}%</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-3 h-3 bg-orange-500 rounded-full"></div>
                  <span>USDC {(tokenBalances.usdc / 
                    (tokenBalances.usdc + (tokenBalances.cbbtc + tokenBalances.lbtc + tokenBalances.wbtc) * 95000) * 100).toFixed(1)}%</span>
                </div>
              </div>
            </div>
            ...
            {/* Token Balances Card */}
            <div className="bg-white rounded-2xl p-8 shadow-sm">
              <h3 className="text-lg font-semibold text-gray-700 mb-6">Token Balances</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-semibold text-gray-900">
                    ${tokenBalances.usdc.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-3">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm">
              ...
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  You can buy up to ${usdcBalance.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                </p>
              </div>
